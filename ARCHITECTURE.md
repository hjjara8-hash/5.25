...
- **Generation Timer Relocation:** Moved the `GenerationTimer` from the input area to the `ChatHeader.tsx`, where it renders conditionally during `isLoading`.
- **Layered Layout:**
...
- **Priority-Based Action Buttons:** Implemented a priority system for the right-side action buttons:
    ...
    - **Priority 3:** `MicrophoneIcon` + `FlowRightIcon` (if `isCharacterMode`).
