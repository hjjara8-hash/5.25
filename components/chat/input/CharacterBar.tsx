            ...
            <div className="flex flex-wrap gap-2 items-center">
                ...
                {characters.map((char) => (
                    <Button 
                        ...
                        className={`px-3 py-1.5 text-xs font-medium bg-fuchsia-600 dark:bg-fuchsia-500 text-white rounded-xl disabled:opacity-50 transition duration-200 ease-out hover:scale-105 ${isReorderingActive ? 'cursor-grab hover:ring-2 hover:ring-fuchsia-400' : 'disabled:cursor-not-allowed'} ${draggedCharRef.current?.id === char.id ? 'opacity-50 ring-2 ring-fuchsia-500' : ''}`}
                    >
                        {char.name}
                    </Button>
                ))}
