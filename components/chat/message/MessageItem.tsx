  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-5xl w-full sm:w-auto`}>
        {isModel && message.isStreaming && !isError && !extractedThoughts && !displayContent && (<div className={`flex items-center space-x-1.5 mb-1.5 px-3 py-2 rounded-2xl shadow-sm ${message.characterName ? 'bg-purple-900/30 border border-purple-500/20' : 'bg-white/5 border border-white/5'}`} aria-label="AI is thinking" role="status"><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce"></div><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce delay-75"></div><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce delay-150"></div></div>)}
