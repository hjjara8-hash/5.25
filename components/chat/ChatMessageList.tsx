...
const ChatMessageList = memo(forwardRef<ChatMessageListHandles, ChatMessageListProps>(({ onEnterReadMode }, ref) => {
    ...
    useEffect(() => {
        if (visibleMessages.length > 0) {
            const currentLastMessage = visibleMessages[visibleMessages.length - 1];
            ...
            if (lastMessageIdRef.current !== currentLastMessage.id) {
                ...
                // Always re-pin and scroll to bottom for new messages
                isPinnedRef.current = true;
                requestAnimationFrame(() => {
                    virtualizer.scrollToIndex(visibleMessages.length - 1, { align: 'end' });
                });
            } else if (!isStreaming && isPinnedRef.current) {
                // If we are just updating an existing message and we are pinned
                requestAnimationFrame(() => {
                    virtualizer.scrollToIndex(visibleMessages.length - 1, { align: 'end' });
                });
            }
    ...
    useEffect(() => {
        ...
        const observer = new ResizeObserver(() => {
            if (isPinnedRef.current && isStreaming && visibleMessages.length > 0) {
    ...
    return (
        <div 
            ...
            className={`flex-1 p-3 sm:p-4 md:p-6 overflow-y-auto relative`} 
        ...
        >
            ...
            {currentChatSession ? (
                visibleMessages.length > 0 ? (
                    <div ref={virtualizerContainerRef} style={{ height: `${virtualizer.getTotalSize()}px`, width: '100%', position: 'relative' }}>
