...
const EditMessagePanel: React.FC = memo(() => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-40 flex justify-center items-center p-4" role="dialog" aria-modal="true" aria-labelledby="edit-message-panel-title" onClick={handleCancelClick}>
      <div 
        className={`bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/10 p-5 sm:p-6 rounded-xl shadow-2xl w-full sm:max-w-2xl max-h-[90vh] flex flex-col text-gray-900 dark:text-gray-100 relative overflow-hidden transition ${isDragging ? 'ring-2 ring-emerald-500 bg-gray-50 dark:bg-black/40' : ''}`}
